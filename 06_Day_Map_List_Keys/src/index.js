...
import hexColors from './hexColors.js'
...
const NewHexColors = ({colors}) =>
{ 
  const grid = colors.map((color) => <NewHexColor color={color}/>)
  return <div class='flex-container'>{grid}</div> 
}
...
const App = () =>
{
  ...
  <div className='app'>
    ...
    <HexaColor color={hexaColor()}/>
    <HexaColor color={hexaColor()}/>
    <NewHexColors colors={hexColors(32)}/>
    <UserCard />
    <Footer />
  </div>
  )
